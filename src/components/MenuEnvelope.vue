...
<template>
  <div>
    <Actions menu-align="right" event="" @click.native.prevent>
      ...
      <template v-if="moreActionsOpen">
        ...
        >
          <template #icon>
            <ChevronLeft :title="t('mail', 'More actions')" :size="20" />
            {{ t("mail", "More actions") }}
          </template>
        </ActionButton>
        <ActionButton
...
export default {
  ...
  computed: {
    ...
    threadingFile() {
      return `data:text/plain;base64,${Base64.encode(
        JSON.stringify(
          {
